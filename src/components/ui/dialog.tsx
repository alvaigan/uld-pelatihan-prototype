    ...
    <>
      {open && (
        <div className="fixed inset-0 z-50 bg-black/50 backdrop-blur-sm" onClick={() => onOpenChange?.(false)}>
          <div className="fixed left-[50%] top-[50%] translate-x-[-50%] translate-y-[-50%]" onClick={(e) => e.stopPropagation()}>
            {children}
          </div>
        </div>
      )}
...
const DialogContent = React.forwardRef<HTMLDivElement, DialogContentProps>(
  ({ className, children, ...props }, ref) => (
    <div
      ref={ref}
      className={cn(
        "grid w-full max-w-4xl gap-4 border bg-white p-6 shadow-lg rounded-lg",
        className
      )}
      {...props}
    >
...
const DialogClose: React.FC<{ onClick?: () => void; children?: React.ReactNode }> = ({ onClick, children }) => (
  <button
    onClick={onClick}
